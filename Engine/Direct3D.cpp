...
namespace Direct3D
{
	...
	XMFLOAT4 lightPos = { 0,0.5,0,0 };//全体の光源ベクトル
}
...
HRESULT Direct3D::InitShaderToon()
{
	...
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&(shaderBundle[SHADER_TOON].pVertexShader_));
	if (FAILED(hr)) {
		MessageBox(NULL, L"頂点シェーダの作成に失敗しました", L"エラー", MB_OK);
		return hr;
	}
	...
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TOON].pPixelShader_);
	if (FAILED(hr)) {
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗しました", L"エラー", MB_OK);
		SAFE_RELEASE(pCompileVS);
		return hr;
	...
	rdc.FillMode = D3D11_FILL_SOLID;   //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE;       //反時計回りを表にするかどうか(がfalseなので時計周りが表)
	hr = pDevice->CreateRasterizerState(&rdc, &(shaderBundle[SHADER_TOON].pRasterizerState_));
	if (FAILED(hr)) {
		MessageBox(NULL, L"ラスタライザの作成に失敗しました", L"エラー", MB_OK);
		return hr;
	}
...
HRESULT Direct3D::InitShaderOutLine()
{
	...
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&(shaderBundle[SHADER_OUTLINE].pVertexShader_));
	if (FAILED(hr)) {
		MessageBox(NULL, L"頂点シェーダの作成に失敗しました", L"エラー", MB_OK);
		return hr;
	}
	...
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_OUTLINE].pPixelShader_);
	if (FAILED(hr)) {
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗しました", L"エラー", MB_OK);
		SAFE_RELEASE(pCompileVS);
		return hr;
	...
	rdc.FillMode = D3D11_FILL_SOLID;   //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE;       //反時計回りを表にするかどうか(がfalseなので時計周りが表)
	hr = pDevice->CreateRasterizerState(&rdc, &(shaderBundle[SHADER_OUTLINE].pRasterizerState_));
	if (FAILED(hr)) {
		MessageBox(NULL, L"ラスタライザの作成に失敗しました", L"エラー", MB_OK);
		return hr;
	}
...
HRESULT Direct3D::InitShaderNormalMap()
{
	...
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&(shaderBundle[SHADER_NORMALMAP].pVertexShader_));
	if (FAILED(hr)) {
		MessageBox(NULL, L"頂点シェーダの作成に失敗しました", L"エラー", MB_OK);
		return hr;
	}
	...
	D3D11_INPUT_ELEMENT_DESC layout[] = {
		...
		{ "NORMAL",	  0, DXGI_FORMAT_R32G32B32_FLOAT,   0, sizeof(DirectX::XMVECTOR) * 2,  D3D11_INPUT_PER_VERTEX_DATA, 0 },//法線
		{ "TANGENT",  0, DXGI_FORMAT_R32G32B32A32_FLOAT,0, sizeof(DirectX::XMVECTOR) * 3,  D3D11_INPUT_PER_VERTEX_DATA, 0 },//接線
	};
	...
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_NORMALMAP].pPixelShader_);
	if (FAILED(hr)) {
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗しました", L"エラー", MB_OK);
		SAFE_RELEASE(pCompileVS);
		return hr;
	...
	rdc.FillMode = D3D11_FILL_SOLID;   //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE;       //反時計回りを表にするかどうか(がfalseなので時計周りが表)
	hr = pDevice->CreateRasterizerState(&rdc, &(shaderBundle[SHADER_NORMALMAP].pRasterizerState_));
	if (FAILED(hr)) {
		MessageBox(NULL, L"ラスタライザの作成に失敗しました", L"エラー", MB_OK);
		return hr;
	}
...
{
	//背景の色
	//float clearColor[4] = { 173/255.0, 216/255.0, 230/255.0, 1.0f };//R,G,B,A
